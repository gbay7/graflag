...
\begin{abstract}%
GraFlag is an open-source Python framework for \emph{reproducible} benchmarking of graph anomaly detection (GAD) methods on distributed infrastructure. While existing libraries such as PyGOD provide detection algorithms and benchmarks like BOND define evaluation protocols, reproducibility of reported results remains fragile: no integrated platform combines containerized execution, standardized result formats, and automated evaluation across distributed GPU clusters. GraFlag addresses this gap through a modular architecture built on Docker Swarm with NFS-based shared storage and on-demand dataset fetching from original sources via versioned per-dataset metadata, so that every experiment can be replayed from a single command on a clean checkout. The framework supports detection methods covering static and dynamic graphs, paired with benchmark datasets. Nine standardized result types ensure consistent evaluation regardless of graph type or detection granularity, making results directly comparable across methods and re-runs. GraFlag provides both a command-line interface and a web dashboard for experiment management, real-time monitoring, and automatic generation of evaluation metrics and publication-ready plots. The framework is available at \url{https://github.com/lias-laboratory/graflag} and installable via \texttt{pip install graflag}.
\end{abstract}
...
\begin{figure*}[t]
   \centering
   \resizebox{0.78\textwidth}{!}{\input{arch}}
...
\textbf{Standardized Result Types.} Regardless of the integration pattern, every method must produce results in one of nine standardized formats, defined as the Cartesian product of three granularities (node, edge, graph) and three temporal modes (static, temporal, streaming), as shown in Table~\ref{tab:result_types}.

\begin{table}[!ht]
\centering
\caption{Standardized result types. Static: 1D vectors; temporal: 2D matrices (entities $\times$ timestamps); streaming: sparse 1D over continuous timeline.}
\label{tab:result_types}
\small
\begin{tabular}{lccc}
\toprule
\textbf{Granularity} & \textbf{Static} & \textbf{Temporal} & \textbf{Streaming} \\
\midrule
Node  & \checkmark & 2D matrix & Sparse 1D \\
Edge  & \checkmark & 2D matrix & Sparse 1D \\
Graph & \checkmark & 2D matrix & Sparse 1D \\
\bottomrule
\end{tabular}
\end{table}
...
gf = GraFlag()
gf.register_metric(
    "EDGE_STREAM_ANOMALY_SCORES", hits_at_100)
...
for m in gf.list_methods():
    for d in gf.list_datasets():
        ...
        print(f"{m.name}/{d.name}: "
              f"AUC={res.metrics['auc_roc']:.3f}")
\end{pycode}
\end{figure}

\textbf{Workflow and Interfaces.} The entire pipeline---from cluster setup through execution to evaluation---is exposed through three interfaces: a CLI (\texttt{graflag setup}, \texttt{graflag run}, \texttt{graflag evaluate}), the Python API shown above, and a web dashboard (\texttt{graflag gui}) with live status badges, log streaming, and one-click evaluation.

\textbf{Reproducibility.} Every experiment persists its full configuration in a JSON file. Any past experiment can be re-executed with \texttt{graflag run -{}-from-config}, with optional parameter overrides.
...
GPU scheduling\footnote{Automatic assignment of containers to GPU-enabled worker nodes via Docker Swarm's generic resource scheduler, as opposed to simply using a locally available GPU.}     & \checkmark & $\times$ & $\times$ & $\times$ & $\times$ \\
...
\subsection{Pattern A: Custom Training Script}

For methods that require a custom training script, three files are needed: a \texttt{.env} configuration, a \texttt{Dockerfile}, and an integration script whose entry point is specified by the \texttt{COMMAND} field.

\textbf{Step 1: Method configuration (\texttt{.env}).} The \texttt{.env} file defines method metadata and default parameters. Parameters prefixed with an underscore are injected as environment variables into the container. When the container entry point uses the \texttt{-{}-pass-env-args} flag, these variables are converted to command-line arguments (e.g., \texttt{\_MAX\_EPOCH=200} becomes \texttt{-{}-max\_epoch~200}).

The \texttt{COMMAND} field specifies the executable invoked inside the container.

The optional \texttt{SUPPORTED\_DATASETS} field lists comma-separated patterns used by the web dashboard to filter compatible datasets; wildcards are expanded to regular expressions (e.g., \texttt{bond\_*} matches \texttt{bond\_.*}). This field affects only the dashboard---any method can be run on any dataset via the CLI.
...
\begin{itemize}[nosep,leftmargin=*]
\item \textbf{ROC Curve}: FPR vs.\ TPR with AUC annotation.
\item \textbf{Precision-Recall Curve}: Recall vs.\ Precision with AUC-PR annotation.
\item \textbf{Score Distribution}: Overlaid histograms of anomaly scores for normal and anomalous entities.
\end{itemize}
...
The dashboard communicates with the cluster through the same \texttt{GraFlag} Python API used by the CLI, ensuring consistent behavior. Background threads poll for state changes every two seconds and push updates to connected clients via WebSocket, avoiding page reloads. All long-running operations (build, run, evaluate) execute in background threads to keep the interface responsive.

\begin{figure}[ht]
\centering
\includegraphics[height=0.45\textheight]{dashboard.png}
