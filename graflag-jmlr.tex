...
\jmlrheading{27}{2026}{1-\pageref{LastPage}}{5/26}{}{26-0000}{Ahmed Youcef Benhalima, Seif-Eddine Benkabou, Amin Mesmoudi and Allel Hadjali}

\ShortHeadings{GraFlag: Reproducible Distributed Benchmarking for Graph Anomaly Detection}{Benhalima, Benkabou, Mesmoudi and Hadjali}
\firstpageno{1}
...
\textbf{On-demand fetching via \texttt{graflag\_data}.} No dataset payload is committed to the repository. Each \texttt{datasets/<name>/} directory ships only a \texttt{metadata.json} file that declares the dataset's original source, the files it needs (with \texttt{url}, an optional \texttt{extract} kind among \texttt{gunzip}, \texttt{zip}, \texttt{tar}, \texttt{tar.gz}, or \texttt{tar.bz2}, an optional \texttt{sha256}, and an optional archive \texttt{members} selector), and---when applicable---a \texttt{build} step that regenerates derived artifacts from a base dataset. The \texttt{graflag\_data} library reads this metadata and hydrates missing files on demand: \texttt{graflag~run} invokes it on the swarm manager via SSH so downloads land directly on the NFS share and are visible to every worker container. HTTP, GitHub raw, and archive sources are handled natively; Google-Drive folders are supported via \texttt{gdown} as an optional extra. Users can also prefetch datasets explicitly with \texttt{graflag-data fetch <name>} or inspect readiness with \texttt{graflag-data status}.
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.9\textheight,keepaspectratio]{dashboard.png}
...
Datasets are hydrated automatically by \texttt{graflag run} via the \texttt{graflag\_data} library (Appendix~\ref{app:datasets}). The same library ships a companion CLI, invoked directly as \texttt{graflag-data <list$|$status$|$fetch>}, for manual prefetch, readiness checks, or forced re-downloads.
